...
Illustrations are produced per article after the quality gate. When `ILLUSTRATION_ENABLED=true` (the default) but **no** image-provider key is set, articles are still published without illustrations and a warning is logged. Set at least one of the four provider blocks below to turn images on. See [Obtaining Credentials §Image / Illustration Providers](obtaining-credentials.md#image--illustration-providers) for step-by-step signup flows.

| Variable | Default | Description |
|----------|---------|-------------|
| `ILLUSTRATION_ENABLED` | `true` | Master on/off switch for the illustration step. |
| `ILLUSTRATION_DEFAULT_STYLE` | `modern tech illustration, clean lines, professional` | Style suffix appended to every prompt. Per-repo override via `.illustyle`. |
| `ILLUSTRATION_DEFAULT_SIZE` | `1792x1024` | Default dimensions. DALL-E 3 accepts `1024x1024`, `1792x1024`, `1024x1792`. |
| `ILLUSTRATION_DEFAULT_QUALITY` | `hd` | Quality hint. DALL-E 3: `standard` or `hd`. |
| `ILLUSTRATION_DIR` | `./data/illustrations` | Filesystem directory for generated image files. |
| `IMAGE_PROVIDER_PRIORITY` | `dalle,stability,midjourney,openai_compat` | Order providers are tried; first with its keys set becomes primary. |
| `OPENAI_API_KEY` | *(empty)* | Enables the **DALL-E 3** provider (recommended minimum). |
| `STABILITY_AI_API_KEY` | *(empty)* | Enables the **Stability AI** (SDXL) provider. |
| `MIDJOURNEY_API_KEY` + `MIDJOURNEY_ENDPOINT` | *(empty)* | Enables the **Midjourney** proxy provider (both required; no defaults). |
| `OPENAI_COMPAT_API_KEY` + `OPENAI_COMPAT_BASE_URL` | *(empty)* | Enables an **OpenAI-compatible** endpoint (Venice, Together, etc.). `OPENAI_COMPAT_MODEL` is optional. |

> **Minimal illustration setup:** put a single `OPENAI_API_KEY=...` in `.env` and leave everything else at defaults.
...
| `OPENAI_API_KEY` | [OpenAI Platform > API keys](https://platform.openai.com/api-keys) (enables DALL-E 3) |
| `STABILITY_AI_API_KEY` | [Stability Platform > API Keys](https://platform.stability.ai/account/keys) |
| `MIDJOURNEY_API_KEY` + `MIDJOURNEY_ENDPOINT` | From your chosen Midjourney proxy (no official API) |
| `OPENAI_COMPAT_API_KEY` + `OPENAI_COMPAT_BASE_URL` | Any OpenAI-compatible image endpoint (Venice, Together, self-hosted) |
