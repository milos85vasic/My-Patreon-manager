...
func newTestClient(t *testing.T, serverURL string) *Client {
	t.Helper()
	oauth := NewOAuth2Manager("test-client-id", "test-client-secret", "test-access-token", "test-refresh-token")
	oauth.SetTokenEndpoint(serverURL + "/api/oauth2/token")
	client := NewClient(oauth, "test-campaign-id")
...
func TestClient_SetPublicationMode(t *testing.T) {
	oauth := NewOAuth2Manager("a", "b", "c", "d")
	client := NewClient(oauth, "campaign123")
	client.SetPublicationMode("public")
	// No getter, just ensure no panic
}

func TestClient_CampaignID(t *testing.T) {
	oauth := NewOAuth2Manager("a", "b", "c", "d")
	client := NewClient(oauth, "campaign123")
	assert.Equal(t, "campaign123", client.CampaignID())
}
