...
// through bootstrapSchemaMigrations so the first run doesn't try to
// re-apply every file against an already-populated schema.
func (db *PostgresDB2) Migrate(ctx context.Context) error {
	m := db.NewMigrator()
	if err := bootstrapSchemaMigrations(ctx, db.db, DialectPostgres, m); err != nil {
...
func (db *PostgresDB2) ContentRevisions() ContentRevisionStore {
